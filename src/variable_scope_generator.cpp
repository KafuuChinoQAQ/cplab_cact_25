...
namespace cplab_variable_scope_generator 
{
    ...
    {
        printf("Processing AST node: %s\n", node.name.c_str()); // 调试输出当前处理的AST节点名称
        printf("Current scope: %s\n", current_scope->name.c_str()); // 调试输出当前作用域名称
        printf("\n");
        ...
        if (node.name == "block")
        {
            ...
            new_scope.name = "Block_" + std::to_string(current_scope->children.size()); // 设置新作用域名称
            ...
            current_scope = &new_scope; // 更新当前作用域为新创建的作用域
        ...
        else if (node.name == "constant_definition" || node.name == "variable_definition")
        {
            ...
            //打印当前作用域的全部标识符
            printf("Current scope identifiers:\n");
            for (const auto &identifier : current_scope->identifiers) {
                printf("  - %s\n", identifier.name.c_str()); // 打印标识符名称
            }
            printf("\n\n");
            // 将当前AST节点的作用域设置为当前作用域
            node.scope_ptr = current_scope;
        ...
        else if (node.name == "function_definition")
        {
            ...
            // // 获取函数形参列表
            // id.func_params.clear(); // 清空函数参数列表
            // ast_node formal_params_node = node.children[3]; // 获取函数形参列表节点
