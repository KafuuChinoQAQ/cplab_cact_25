...
class bit_vector {
private:
  static constexpr size_t SOO_THRESHOLD =
      64 * 3; // Threshold for small object optimization
  std::variant<std::bitset<SOO_THRESHOLD>, std::vector<uint64_t>> data;
  size_t size_;

public:
  bit_vector() = default;
  explicit bit_vector(size_t size) : size_(size) {
  ...
  explicit bit_vector(size_t size, bool value) : size_(size) {
    if (size > SOO_THRESHOLD) {
  ...
  static bit_vector empty_set(size_t size) { return bit_vector(size, false); }

  static bit_vector full_set(size_t size) { return bit_vector(size, true); }

  bit_vector(const bit_vector &other) : size_(other.size_) {
  ...
  bit_vector(bit_vector &&other) noexcept : size_(other.size_) {
    if (std::holds_alternative<std::vector<uint64_t>>(other.data))
  ...
  bit_vector &operator&=(const bit_vector &other) {
    if (size_ != other.size_) {
      throw std::invalid_argument(
          "bit_vectors must be of the same size for bitwise operation");
    }
    if (std::holds_alternative<std::vector<uint64_t>>(data) &&
        std::holds_alternative<std::vector<uint64_t>>(other.data)) {
      for (size_t i = 0; i < std::get<std::vector<uint64_t>>(data).size(); ++i)
        std::get<std::vector<uint64_t>>(data)[i] &=
            std::get<std::vector<uint64_t>>(other.data)[i];
    } else
      std::get<std::bitset<SOO_THRESHOLD>>(data) &=
          std::get<std::bitset<SOO_THRESHOLD>>(other.data);
    return *this;
  }
  ...
  bit_vector operator~() const {
    ...
    if (std::holds_alternative<std::vector<uint64_t>>(data))
      for (size_t i = 0; i < std::get<std::vector<uint64_t>>(data).size(); ++i)
        std::get<std::vector<uint64_t>>(result.data)[i] =
            ~std::get<std::vector<uint64_t>>(data)[i];
    else
      std::get<std::bitset<SOO_THRESHOLD>>(result.data) =
          ~std::get<std::bitset<SOO_THRESHOLD>>(data);
    return result;
  }
  ...
  bool operator==(const bit_vector &other) const {
    ...
    if (std::holds_alternative<std::vector<uint64_t>>(data) &&
        std::holds_alternative<std::vector<uint64_t>>(other.data)) {
      return std::get<std::vector<uint64_t>>(data) ==
             std::get<std::vector<uint64_t>>(other.data);
    }
  ...
  public:
    iterator(const bit_vector &bv, size_t start_pos) : bv(bv), pos(start_pos) {
      if (std::holds_alternative<std::vector<uint64_t>>(bv.data)) {
        word_index = start_pos / 64;
        current_word = bv[word_index];
      }
      while (pos < bv.size() && !bv[pos])
